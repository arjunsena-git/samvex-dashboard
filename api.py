...
DZ_IMPULSE_VOL_RATIO  = 1.8   # impulse candle's volume vs the avg volume of the days before the base — rolled back 2026-07-13, see PDH_VOL_MIN note
...
def _screen_exhaustion_short(_debug: dict = None) -> list:
    ...
    # Only generate fresh shorts between 9:15 AM and 2:30 PM IST
    if not (_dtime(9, 15) <= now.time() <= _dtime(14, 30)):
        if _debug is not None:
            _debug["blocked"] = "Exhaustion Short only fires 9:15 AM–2:30 PM IST — outside that window right now."
        return []
...
PDH_RSI_PERIOD = 14
PDH_RSI_MIN    = 60.0
PDH_VOL_MIN    = 1_000_000   # 10 lakh shares — rolled back 2026-07-13, auto-tuner ratcheted 1.0M→2.0M over 3wks with no win-rate gain
...
ORB_VOL_RATIO             = 1.5    # breakout candle volume vs avg volume of the consolidation bars — rolled back 2026-07-13, see PDH_VOL_MIN note
...
MB_VOL_RATIO          = 1.5   # breakout candle volume vs avg volume of bars so far today — rolled back 2026-07-13, see PDH_VOL_MIN note
...
TRAP_VOL_RATIO        = 1.5   # reversal candle's volume vs avg volume of bars so far today — rolled back 2026-07-13, see PDH_VOL_MIN note
...
def admin_dhan_totp_diag():
    ...
    try:
        ...
        # Never echo the PIN/TOTP (they're in the request URL) or the token
        # itself (it's in the response body) back in a diagnostic response —
        # both are live credentials, not debug info.
        redacted_body = re.sub(r'"accessToken"\s*:\s*"[^"]+"', '"accessToken":"[redacted]"', r.text)
        diag["response_body"] = redacted_body[:500]
