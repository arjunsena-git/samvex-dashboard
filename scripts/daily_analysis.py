...
# 2026-07-13: paused after a ~4-week one-way ratchet (never once loosened —
# WIN_RATE_HIGH_THRESHOLD is nearly unreachable off tiny single-day samples
# when true win rate runs ~10-15%) tightened volume thresholds nearly every
# day and collapsed daily signal volume from ~60-70 to 0-7 with NO win-rate
# improvement to show for it (still ~0-10% at the lowest volume). Flip back
# to True only once satisfied the guardrails below actually hold the floor.
...
def fetch_history_signals():
    """Pulls the dashboard's own rolling signal-outcome history (up to 30
...
def apply_code_improvement(report):
    ...
    for setup_num, tuner in PANEL_TUNERS.items():
        ...
        if new == old and "fresh_const" in tuner:
            # Primary knob already saturated (at its ceiling while tightening,
            # or its floor while loosening) — escalate to the freshness knob
        ...
        if new == old:
            continue  # already at floor/ceiling and can't move further that direction
    ...
    narrative = (
        ...
        + "\nGoal: each panel's volume/freshness gate moves independently off its OWN trailing "
          ...
          "so tuning can't repeat the volume collapse this was paused for.\n"
